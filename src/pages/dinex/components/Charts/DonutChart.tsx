...
// Componente base: DonutChart Simple (solo muestra datos)
...
export const DonutChartWithFilters = ({
    title = "Chart Title",
    ...
    showDownloadButton = true,
    filters = [
        { id: 'filter1', label: 'Filter 1', series: [15.1, 22.5, 4.4, 8.4] },
        { id: 'filter2', label: 'Filter 2', series: [25.1, 26.5, 1.4, 3.4] },
        { id: 'filter3', label: 'Filter 3', series: [45.1, 27.5, 8.4, 2.4] }
    ],
    timeRangeLabel = "Last 7 days",
    actionLabel = "View Details",
    actionHref = "#",
    ...
    const [currentSeries, setCurrentSeries] = useState(defaultSeries);
    const [isDarkMode, setIsDarkMode] = useState(false);
    ...
    useEffect(() => {
        ...
        if (chartRef.current && window.ApexCharts) {
            chartInstanceRef.current = new window.ApexCharts(
            chartRef.current,
            getChartOptions(defaultSeries, isDarkMode)
            );
            chartInstanceRef.current.render();
        }
        };
    ...
    // Actualizar el gráfico cuando cambie el modo oscuro
    useEffect(() => {
        if (chartInstanceRef.current && window.ApexCharts) {
        try {
            chartInstanceRef.current.updateOptions(getChartOptions(currentSeries, isDarkMode), false, true);
        } catch (error) {
            console.error('Error updating chart:', error);
        }
        }
    ...
    const handleFilterChange = (filterId) => {
        const newSelectedFilter = selectedFilter === filterId ? null : filterId;
        setSelectedFilter(newSelectedFilter);

        let newSeries;
        if (newSelectedFilter) {
        const filter = filters.find(f => f.id === filterId);
        newSeries = filter ? filter.series : defaultSeries;
        } else {
        newSeries = defaultSeries;
        }
        ...
        if (chartInstanceRef.current && window.ApexCharts) {
        try {
            chartInstanceRef.current.updateSeries(newSeries);
        } catch (error) {
        ...
        if (onFilterChange) {
        onFilterChange(newSelectedFilter, newSeries);
        }
    };

    const handleDownloadClick = () => {
        if (onDownload) {
        onDownload(currentSeries, selectedFilter);
        }
    };
